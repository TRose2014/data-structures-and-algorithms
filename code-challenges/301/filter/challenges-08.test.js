...
const notInFirstArray = (forbiddenValues, arr) => {
  let answer = arr.filter((element) => ! forbiddenValues.includes(element));
  return answer;
};
...
const getStatName = (arr, minBaseStat) => {
  let attackObj = arr.filter((element) => {
    return element.baseStat > minBaseStat;
  });

  let answer = attackObj.map(n => n.stat.name);
  return answer;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const getCharactersWithoutChildren = (arr) => {
  let answer = arr.filter((element) => !element.children);
  return answer;
};
